            ...
            <form action="{{ route('admin.konfigurasi.update', $konfigurasi->id) }}" method="POST"
                ...
                <div>
                    ...
                    <div class="space-y-4 mt-4">
                        ...
                        <div x-data="{ preview: null }">
                            ...
                            @if ($konfigurasi->logo)
                                <img x-show="!preview" src="{{ asset('storage/' . $konfigurasi->logo) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input name="nama_pt" class="w-full mt-1"
                                    placeholder="PT. PLN (Persero) ULP Banjarbaru" :value="old('nama_pt', $konfigurasi->nama_pt)" />
                            </div>

                            <div>
                                <x-input-label value="Nama Aplikasi*" />
                                <x-text-input name="nama_apk" class="w-full mt-1" placeholder="APM" :value="old('nama_apk', $konfigurasi->nama_apk)" />
                            </div>
                        </div>
                ...
                <div>
                    ...
                    @php
                        $selected = old(
                            'hari_kerja',
                            is_array($konfigurasi->hari_kerja)
                                ? $konfigurasi->hari_kerja
                                : json_decode($konfigurasi->hari_kerja, true) ?? [],
                        );
                    @endphp
                    ...
                    <div class="mt-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-text-input type="number" step="any" name="kantor_lat" class="w-full mt-1"
                                    placeholder="-3.12457854" :value="old('kantor_lat', $konfigurasi->kantor_lat)" />
                            </div>

                            <div>
                                <x-input-label value="Longitude*" />
                                <x-text-input type="number" step="any" name="kantor_lng" class="w-full mt-1"
                                    placeholder="114.21452154" :value="old('kantor_lng', $konfigurasi->kantor_lng)" />
                            </div>

                            <div>
                                <x-input-label value="Radius Absen ('0' untuk mode WFA)*" />
                                <x-text-input type="number" name="radius" class="w-full mt-1" :value="old('radius', $konfigurasi->radius ?? 0)" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div class="relative mt-1">
                                    ...
                                    <x-text-input name="ig_link" class="w-full pl-40" placeholder="username"
                                        :value="old(
                                            'ig_link',
                                            $konfigurasi->ig_link
                                                ? str_replace('https://instagram.com/', '', $konfigurasi->ig_link)
                                                : '',
                                        )" />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input name="link_maps" class="w-full mt-1"
                                    placeholder="https://www.google.com/maps/..." :value="old('link_maps', $konfigurasi->link_maps)" />
                            </div>
                        ...
                        <div>
                            ...
                            <textarea name="alamat" rows="3"
                                ...
                                placeholder="Jl. Panglima Batur No.10">{{ old('alamat', $konfigurasi->alamat) }}</textarea>
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="radio" name="status_hari" value="libur"
                                    {{ empty($selected) ? 'checked' : '' }} onclick="toggleHari(true)">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="radio" name="status_hari" value="aktif"
                                    {{ !empty($selected) ? 'checked' : '' }} onclick="toggleHari(false)">
                ...
                <div>
                    ...
                    <div class="mt-4">
                        <textarea id="aturan_presensi" name="aturan_presensi" rows="4"
                            ...
                            placeholder="Contoh: Izin tanpa pengajuan hanya 1 hari">{{ old('aturan_presensi', $konfigurasi->aturan_presensi) }}</textarea>
                    </div>
                </div>
